...
class Pouch(object): 
    ...
    def _calc_initiator_count(self):
        
        if self.number_initiator is not None:
            return int(self.number_initiator)
        
        if isinstance(self.frac, (float, int)):                 
            frac_val = float(self.frac)
        elif isinstance(self.frac, str):
            frac_val = self.scalingCoeff * self.n_cells ** (self.scalingPower - 1)
            self.frac=frac_val
        else:                                              
            raise ValueError(f"Invalid frac: {raw_frac}")
            
        return int(frac_val * self.n_cells)
        
    @property
    def initiatorCellCount(self):
        return self._calc_initiator_count()
